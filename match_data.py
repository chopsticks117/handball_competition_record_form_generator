...
def extract_matches_for_date(content, match_type, target_date):
    matches = []
    ...
    if match_type == '聯賽':
        matches = find_leauge_matches(lines, target_year, target_month, target_day)
    else:
        matches = find_hkssf_matches(lines, target_year, target_month, target_day)
    
    return matches

def find_leauge_matches(lines, target_year, target_month, target_day):
    ...
    return laegue_matches

def find_hkssf_matches(lines, target_year, target_month, target_day):
    hkssf_matches = []
    
    # Format the target date as a string for comparison
    ...
    while i < len(lines):
        ...
        if line == target_date_str:
            
            targetDayOfWeek = ""
            # Move to the next line (first match of the day)
            i += 1
            # Skip the day of the week line (e.g., "(日)")
            if i < len(lines) and lines[i].strip().startswith("("):
                targetDayOfWeek = lines[i]
                i += 1
            
            # Extract the location from the first match of the day
            if i < len(lines):
                first_match_line = lines[i].strip()
                if first_match_line:
                    first_match_parts = first_match_line.split()
                    if len(first_match_parts) >= 9:
                        location = first_match_parts[-1]  # Location is the last part of the first match line
                    else:
                        location = "N/A"  # Default if location is missing
            
            # Process all matches for the day
            while i < len(lines):
                match_line = lines[i].strip()
                # Stop if we encounter a new date
                if "/" in match_line and match_line.split("/")[0].isdigit():
                    break
                # Skip empty lines or day of the week lines
                if not match_line or match_line.startswith("("):
                    i += 1
                    continue
                
                # Split the match line into parts
                parts = match_line.split()
                
                # Ensure the line has enough parts to contain match details
                if len(parts) >= 9:
                    time_start = parts[1]
                    time_start_obj = datetime.strptime(time_start, '%H%M')
                    time_end_obj = time_start_obj + timedelta(hours=1)
                    time_end = time_end_obj.strftime('%H%M')
                    
                    match_info = {
                        "場次": parts[0],  # Match number
                        "日期": target_date_str+targetDayOfWeek,  # The date
                        "時間": f"{time_start}-{time_end}",  # Start time
                        "組別": f"{parts[2]} {parts[3]}",  # Group (e.g., "女初 C")
                        "主隊": f"{parts[4]} {parts[5]}",  # First team (e.g., "D3 平安福音")
                        "客隊": f"{parts[7]} {parts[8]}",  # Second team (e.g., "侯寶垣 D4")
                        "成績": parts[9] if len(parts) > 9 else "N/A",  # Score if available
                        "地點": location  # Location is the same as the first match of the day
                    }
                    hkssf_matches.append(match_info)
                i += 1
        else:
            i += 1
    
    return hkssf_matches
